import type { ParsedItem, ParsedItemKind } from '@command-center/shared';

/** Item kinds that support simple ↔ complex toggling (tasks, problems, study items). */
export const COMPLEXITY_TOGGLE_KINDS: Array<ParsedItemKind> = ['task', 'problem', 'study_weakness'];

export const hasComplexityToggleInput = (items: Array<ParsedItem>): boolean =>
  items.some((item) => COMPLEXITY_TOGGLE_KINDS.includes(item.kind));
...
export const MAKE_SIMPLE_TASK_CORRECTION =
  ...
  'Set isSimpleTask to true. Return exactly one item with kind "task" whose title closely mirrors the original text, with an empty suggestedTasks array ([]).';
